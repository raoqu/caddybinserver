...
func (fsrv *BinServer) serveBrowse(root, dirPath string, w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
...
func (fsrv *BinServer) loadDirectoryContents(ctx context.Context, dir fs.ReadDirFile, root, urlPath string, repl *caddy.Replacer) (*browseTemplateContext, error) {
...
func (fsrv *BinServer) browseApplyQueryParams(w http.ResponseWriter, r *http.Request, listing *browseTemplateContext) {
...
func (fsrv *BinServer) makeBrowseTemplate(tplCtx *templateContext) (*template.Template, error) {
...
// is a directory.
func (fsrv *BinServer) isSymlinkTargetDir(f fs.FileInfo, root, urlPath string) bool {
	if !isSymlink(f) {
		return false
	}
