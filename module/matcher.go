...
func (m MatchFile) selectFile(r *http.Request) (matched bool) {
	...
	setPlaceholders := func(candidate matchCandidate, info fs.FileInfo) {
		repl.Set("http.matchers.bin.relative", filepath.ToSlash(candidate.relative))
		repl.Set("http.matchers.bin.absolute", filepath.ToSlash(candidate.fullpath))
		repl.Set("http.matchers.bin.remainder", filepath.ToSlash(candidate.splitRemainder))

		fileType := "file"
		if info.IsDir() {
			fileType = "directory"
		}
		repl.Set("http.matchers.bin.type", fileType)
	}
